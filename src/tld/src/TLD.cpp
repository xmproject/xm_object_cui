...
void TLD::init(const cv::Mat& frame1,const cv::Rect& box,FILE* bb_file){
...
void TLD::generatePositiveData(const cv::Mat& frame, int num_warps){
  ...
  cv::Mat img;
  cv::Mat warped;
  GaussianBlur(frame,img,cv::Size(9,9),1.5);
  warped = img(bbhull);
...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<cv::Point2f>& points1,vector<cv::Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file){
...
void TLD::track(const cv::Mat& img1, const cv::Mat& img2,vector<cv::Point2f>& points1,vector<cv::Point2f>& points2){
  /*Inputs:
  ...
  vector<cv::Point2f> points = points1;
