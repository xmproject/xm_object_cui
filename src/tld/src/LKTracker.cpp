...
using namespace std;
LKTracker::LKTracker(){
  term_criteria = cv::TermCriteria( cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 20, 0.03);
...
bool LKTracker::trackf2f(const cv::Mat& img1, const cv::Mat& img2,vector<cv::Point2f> &points1, vector<cv::Point2f> &points2){
  ...
  for( int i= 0; i<points1.size(); ++i ){
        FB_error[i] = cv::norm(pointsFB[i]-points1[i]);
  }
...
void LKTracker::normCrossCorrelation(const cv::Mat& img1,const cv::Mat& img2, vector<cv::Point2f>& points1, vector<cv::Point2f>& points2) {
...
bool LKTracker::filterPts(vector<cv::Point2f>& points1,vector<cv::Point2f>& points2){
  //Get Error Medians
