...
void FerNNClassifier::read(int sss){
...
void FerNNClassifier::prepare(const vector<cv::Size>& scales){
  acum = 0;
  ...
  for (int i=0;i<nn_examples.size();i++){                          //  For each example
      ...
      if (y[i]==1 && conf<=thr_nn){                                //    if y(i) == 1 && conf1 <= tld.model.thr_nn % 0.65
          if (isin[1]<0){                                          //      if isnan(isin(2))
              pEx = vector<cv::Mat>(1,nn_examples[i]);                 //        tld.pex = x(:,i);
              continue;                                            //        continue;
          }                                                        //      end
...
void FerNNClassifier::NNConf(const cv::Mat& example, vector<int>& isin,float& rsconf,float& csconf){
  /*Inputs:
   * -NN Patch
   * Outputs:
...
void FerNNClassifier::show(){
  cv::Mat examples((int)pEx.size()*pEx[0].rows,pEx[0].cols,CV_8U);
  double minval;
  ...
  for (int i=0;i<pEx.size();i++){
    ...
    pEx[i].copyTo(ex);
    ex = ex-minval;
    cv::Mat tmp = examples.rowRange(cv::Range(i*pEx[i].rows,(i+1)*pEx[i].rows));
    ex.convertTo(tmp,CV_8U);
  }
  cv::imshow("Examples",examples);
}
